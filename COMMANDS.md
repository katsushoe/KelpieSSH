...
- `<file-path>` is a normalized absolute path relative to that site root, such as `/_webadmin/index.php`. A terminal `/*` matches regular files directly in that directory; `/**` matches regular files recursively below it. Root-wide or intermediate globs, empty segments, traversal, the directory itself, and symbolic-link escapes are rejected. Exact entries take precedence over matching directory globs.
...
- `apply` validates every change before preview and rejects duplicate paths. Each manifest entry is applied as an upsert: a missing entry is added, a different access value is updated, and an identical value is unchanged. Mixed changes use one confirmation and one atomic replacement; an entirely unchanged manifest exits successfully without confirmation, backup, replacement, or success audit. Successful `add`, `remove`, `apply`, and `rollback` results report `success: true`; `apply` also reports `changed`, `addedCount`, `updatedCount`, and `unchangedCount`. Any validation or post-replacement failure leaves or restores the prior policy. Restoration uses at most two atomic rename attempts; if both fail, the command returns an error and retains the managed backup for recovery.
...
- Before writing, the command prints the current and proposed JSON as a line-oriented difference and generates a cryptographically random four-digit lowercase hexadecimal confirmation code. Input matching is case-insensitive. EOF, mismatch, redirection, or extra arguments aborts without changing the policy.
- The proposed and backup JSON are parsed and schema-validated. The policy accepts only `Sites.<site-root>.AllowedFiles.<file-path>` with `Update` or `Create`. The helper re-evaluates exact and limited directory-glob entries against the resolved write path; `Create` remains required for a missing file, while `Update` permits only an existing regular file.
...
The command exits with `1` for invalid syntax or JSON, unsupported policy fields or values, unsafe paths, non-interactive execution, non-root execution, insecure ownership or mode, confirmation mismatch, missing entries or backups, metadata preservation failure, backup failure, audit failure, or atomic replacement failure. A single `add` still rejects an existing entry; `apply` instead updates or leaves it unchanged. Expected command failures are written as concise errors without exception types, source paths, line numbers, or stack traces. When the VPS helper does not support the `policy` action, the command reports that helper `0.2.1.0` or later must replace the existing helper while preserving root ownership and mode `0755`. It exits with `0` after the requested read, change, or verified no-op completes.
...
The command validates and hashes the local regular file, uploads it through internal SFTP to a fixed staging path, verifies the remote hash, and displays the current version and hashes. If the hashes already match, it exits successfully without confirmation or replacement and reports `changed: false`. Otherwise it requires a random four-digit lowercase hexadecimal confirmation code with case-insensitive matching. The private fixed-command wrapper then backs up the installed helper, verifies the root-owned temporary copy, enforces `root:root` mode `0755`, atomically replaces the target, verifies its identity, and records completion through `logger`. A successful result contains `success`, `changed`, and `message`. A failed privileged step attempts rollback.
