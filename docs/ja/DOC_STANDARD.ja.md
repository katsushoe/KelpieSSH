...
# 言語とファイル配置ルール
英語ドキュメントは、repository root または対象機能ディレクトリに通常の `.md` として配置する。

日本語ドキュメントは `docs/ja/` 配下に `.ja.md` として配置する。機能ディレクトリの文脈を残したい場合は、`docs/ja/installer/README.ja.md` のように `docs/ja/` 配下でサブディレクトリを維持する。

`.en.md` は使用しない。英語版が公開既定の入口であり、日本語版は対応する英語版へ戻るリンクを必要に応じて置く。

日本語の公開入口は `docs/ja/README.ja.md` とする。
